          ...
          if (error && error.response && error.response.status !== 200) {
            promise.resolve({
              response: {
                ...
                ...error.response.data.response.alert,
                // alert: [
                //   {
                //     message: errorCode,
                //     type: 'failure',
...
axios.interceptors.response.use(
  ...
  (error) => {
    ...
    if (!response) {
      return Promise.reject({
        response: {
          ...
          alert: [
            {
              message: Message.Error.COMMON_MESSAGE,
              type: 'success',
              skippable: 1,
            },
          ],
    ...
    if (
      ...
      if (window.location.pathname === '/otpscreen') {
        // Handle the error on the OTP screen (e.g., show a message)
...
export async function makeHttpRequest(url, method, payload, isStringify) {
  const { data } = payload;
  let { headers } = payload;
  // console.log(headers,!headers,isStringify,  'headers')
  // console.log(`${url}`);
  ...
  return new Promise(function (resolve, reject) {
    ...
    try {
      if (url === null || url === undefined || url === '') {
        ...
        // console.log('====================== Promise start', options);
        // if (
...
export async function update(url, payload = {}) {
  return makeHttpRequest(url, METHOD_PUT, payload, 'upload');
}

export async function uploadAll(url, payloads) {
  return Promise.all(
    payloads.map(
      async (payload) =>
        await makeHttpRequest(url, METHOD_POST, payload, 'upload')
    )
  ).then((data) => {
    return data;
  });
}
export async function updateAll(url, payloads) {
  return Promise.all(
    payloads.map(async (payload) => {
      let newurl = `${url}/${payload?.item?.id}`;
      await makeHttpRequest(newurl, METHOD_PUT, payload, 'upload');
    })
  ).then((data) => {
    return data;
  });
}
export async function updateAllBanner(url, payloads, short_name = '') {
  return Promise.all(
    payloads.map(async (payload) => {
      let newurl = `${url}/${payload?.item?.id}?lang=${short_name}`;
      await makeHttpRequest(newurl, METHOD_PUT, payload, 'upload');
    })
  ).then((data) => {
    return data;
...
export async function updateAllBannersResouces(url, payloads, page) {
  return Promise.all(
    payloads.map(async (payload) => {
      let newurl = `${url}/${payload?.item?.banner_id}/${payload?.item?.id}?lang=${page}`;
